...
public class TimeNumberUtils {
    ...
    public static void main(String[] args) {
        ...
        System.out.println(TimeNumberUtils.getInstance().GenerateOrder());
    }
    // 使用单例模式，不允许直接创建实例
    private TimeNumberUtils() {}

    // 创建一个空实例对象，类需要用的时候才赋值
    private static TimeNumberUtils g = null;

    // 单例模式--懒汉模式
    public static synchronized TimeNumberUtils getInstance() {
        if (g == null) {
            g = new TimeNumberUtils();
        }
        return g;
    }
